...
export default function MainMenu() {
  ...
  return (
    <div className="relative w-full h-screen bg-linear-to-b from-gray-900 via-gray-700 to-blue-900 overflow-hidden flex">
      {stars.map((_, index) => {
      ...
      <div className="w-full flex flex-col justify-center items-center gap-10">
        <button onClick={handlePlayClick} className="relative flex items-center justify-center w-1/5 h-1/10 text-[clamp(12px,2vw,50px)] font-medium tracking-[0.05em] 
                            bg-[#1E3A8A] text-white rounded-[0.9em] border-none
        ...
        <button onClick={handleInventoryClick} className="relative flex items-center justify-center w-1/3 h-1/10 text-[clamp(12px,2vw,50px)] font-medium tracking-[0.05em] 
                            bg-[#112457] text-white rounded-[0.9em] border-none
                            shadow-[inset_0_0_1.6em_-0.6em_#000f42] overflow-hidden
                            cursor-pointer transition-all duration-300
                            hover:bg-[#000f42]">
             Ekwipunek
